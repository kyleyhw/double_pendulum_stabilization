...
class Visualizer:
    ...
    def draw_pendulum(self, state, color_p1=None, color_p2=None, alpha=255):
        ...
        if len(state) == 6:
            # Double: [x, t1, t2, xd, t1d, t2d]
            x, theta1, theta2, _, _, _ = state
        elif len(state) == 8:
            # Double (Trig): [x, sin1, cos1, sin2, cos2, x_dot, t1_dot, t2_dot]
            ...
            # Single: [x, t1, xd, t1d]
            x, theta1, _, _ = state
            theta2 = None # No second pole
        elif len(state) == 5:
             # Single (Trig)? [x, sin, cos, xd, td]
             x, s1, c1, _, _ = state
             theta1 = np.arctan2(s1, c1)
             theta2 = None
        else:
            # Fallback (Try to parse at least 1)
            print(f"Warning: Unknown state dim {len(state)}. Attempting fallback.")
            x = state[0]
            theta1 = state[1] if len(state) > 1 else 0
            theta2 = state[2] if len(state) > 2 else None
        ...
        l1 = self.env.l1 * self.scale if hasattr(self.env, 'l1') else getattr(self.env, 'l', 1.0) * self.scale
        l2 = getattr(self.env, 'l2', 1.0) * self.scale
